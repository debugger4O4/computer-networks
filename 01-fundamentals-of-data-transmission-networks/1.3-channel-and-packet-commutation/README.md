...
Для передачи всех данных абоненту `N2` в сети с коммутацией каналов понадобится `825 мс`.<br>


Подсчет для передачи с коммутацией пакетов с тем же объемомданных `V = 10^7 байт`:
- `C = 100 Мбит/c`, где `C` - пропускная способность линий связи;
- Число промежуточных коммутаторов `(S1 - S10) = 10`;
- Исходное сообщение разбивается на пакеты по `10^3 байт` с заголовками `40 байт`;
- Интервалы `(t1)` между всеми пакетами = `100 мкс`;
- Времена коммутации `(t7)` на каждом коммутаторе = `50 мкс`.

`T_N1-S1 = интервалы между всеми паетами + t4(1) + 83 мкс + времена коммутации на каждом коммутаторе = 233 мкс + t4(1)`.<br>
`T_S1-S2 = 133 мкс + t4(2)`.<br>
`T_S1-S2` - время передачи первого пакета от коммутатора `S1` до коммутатора `S2`. Отличается `t1 = 100 мкс` (пакет уже 
сформирован), `t4` (`t4(2)` - зависит от `L2` между `S1` и `S2`).<br>
`T_Si-Si+1 = 133 мкс + t4(i+1)`, где `1 <= i <= 10`.<br>
`T_S10-N2 = 83 мкс + t4(11)`.<br>
`T_S10-N2` - время передачи первого пакета от коммутатора `S10` до конечного узла `N2`. Отличается отсутствием `t7 = 50 мкс`
(в конечном узле отсутствует операция коммутации) и `t4(11)`.<br>
`T1 = T_N1-S1 + T_S1-S2 + ... + T_S10-N2 = 233 мкс + (133 мкс) * 9 + 83 мкс (буферизация) * ∑t4(i) = 1513 мкс + (∑Li)/S`.<br>
`T1` - суммарное время прохождения первого пакета от узла `N1` до узла `N2`.<br>
Полное время передачи первого пакета:<br>
`T1 = T_N1-N2 = 0.001513 с + (5000 км) / (200000 км/c) = 0.001513 c + 0.025 c = 0.026513 c`.<br>
10000 пакетов:<br>
`T_PS = T1 + (10000 - 1)(t1 + t5) = 0.026513 + 9999 * (0.0001 + 0.000083) = 0.026513 + 9999 * 0.000183 = 0.026513 + 1.829817 = 1.85633 с = 1856 мс`.<br>
Передача данных по сети с коммутацией пакетов с указанными условиями на `1031 мс` дольше, чем в сети с коммутацией каналов.
### Ethernet - пример стандартной технологии с коммутацией пакетов.
**Ethernet** - одна из первых стандартных сетевых технологий. Битовая скорость = `10 Мбит/с`.<br>
- *Топология*. Существует два варианта Ethernet: 
  - на разделяемой среде (все узлы сети разделяют общую среду передачи данных, т.е. сеть строится по топологии общей шины).
<br><img alt="img16.png" height="200" src="content%2Fimg16.png" width="300"/><br> 
  - коммутируемый вариант.
<br><img alt="img17.png" height="200" src="content%2Fimg17.png" width="300"/><br> 
- *Способ коммутации*. Дейтаграммная коммутация пакетов. Единицы данных называют кадрами - имеет фиксированный формат и
содержит различную служебную информацию.
- *Адресация*. Каждый сетевой адаптер имеет уникальный аппаратный адрес (MAC-адрес). Ethernet-адрес является плоским 
числовым адресом. Поддерживаются адреса для индивидуальной, широковещательной и групповой рассылки.
- *Разделение среды и мультиплексирование* - каждый канал являтся дуплексным. Передатчики используют дуплексные каналы связи
для мультиплексирования потоков кадров. Конечные узлы применяют специальный метод доступа с целью синхронизации использования 
единственного полудуплексного канала связи, объединяющего все компьютеры сети. Единого арбитра на разделяемой среде нет.
Узлы прибегают к распределенному случайному методу доступа. Информационные потоки, поступающие от конечных узлов сет, мультиплексируются
в единственном передающем канале в режиме разделения времени, т.к. кадрам разных потоков поочередно предоставляется канал.
- *Кодирование*. Адаптеры работают с тактовой частотой `20 МГц`, передавая в среду прямоугольные импульсы, соответствующие 
единицам и нулям данных компьютера. Скорость передачи битов = `10 Мбит/c` (каждый бит передается за два такта).
- *Надежность*. Подсчет контрольной суммы и ее передача в конце кадра. При обнаружении ошибки кадр отбрасывается, повторная
передача не выполняется.
- *Очереди*. Переменная задержка доставки кадров.
